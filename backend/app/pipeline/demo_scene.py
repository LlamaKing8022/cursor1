...
class DemoBeachScene:
    ...
    Animation runs off an internal clock advanced by `step(dt)` rather than
    wall-clock time, so the same scene renders correctly for a live feed and for
    ...
    def force_distress(self) -> DemoSwimmer | None:
        """Put one ordinary swimmer into a distress pattern immediately."""
    ...
    def render(self, detections: list[Detection] | None = None) -> np.ndarray:
        ...
        for row in range(int(h * 0.38)):
            t = row / max(h * 0.38, 1)
            frame[row, :] = (int(210 - 40 * t), int(170 - 20 * t), int(120 + 30 * t))

        ocean_top = int(h * 0.38)
        for row in range(ocean_top, h):
            t = (row - ocean_top) / max(h - ocean_top, 1)
            frame[row, :] = (int(150 - 40 * t), int(110 + 20 * t), int(40 + 10 * t))
